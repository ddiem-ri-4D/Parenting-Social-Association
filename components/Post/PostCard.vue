<template>
  <div class="post-card">
    ...
    <div class="bar">
      ...
      <nuxt-link :to="'/post/' + post._id" class="bar__item">
        <a-icon type="message" class="bar__item-icon" />
        <p>{{ post.reviews.length }} {{ $t('Bình luận') }}</p>
      </nuxt-link>
      <div class="bar__item" @click="handleSave">
        <a-icon type="save" class="bar__item-icon" />
        <p v-if="$auth.user.deckShare.map((item) => item._id).indexOf(post._id) === -1">{{ $t('Lưu bài viết') }}</p>
        <p v-else>{{ $t('Bỏ lưu') }}</p>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async handleLike(id) {
      try {
        const result = await this.$axios.$post('/decks/' + id)
        if (result.success) {
          this.fetchLike(id)
        }
      } catch (error) {
    ...
    async fetchLike(id) {
      try {
        const result = await this.$axios.$get('/decks/' + id)
    ...
    handleSave() {
      this.$axios
        .$post('/api/auth/user/' + this.post._id)
        .then((res) => {
          if (res.success) {
    ...
    async onAddReview() {
      if (this.body) {
        try {
          ...
          const response = await this.$axios.$post(`/reviews/${this.deck._id}`, data)
    ...
    handleDelete() {
      this.$axios
        .$delete('/decks/' + this.deck._id)
        .then((res) => {
          if (res.success) {
            this.$notification.success({
              message: this.$t('Thông báo'),
              description: this.$t('Xóa thành công'),
            })

            this.$emit('fetchDelete', this.deck._id)
          }
        })
