    ...
    <div class="options">
      <div class="left">
        <a-breadcrumb>
          ...
          <a-breadcrumb-item>{{ $t('Phụ huynh') }}</a-breadcrumb-item>
        </a-breadcrumb>
      </div>

      <div class="right">
        <a-button type="primary" @click="handleOpenModalAdd"><a-icon type="plus" /> {{ $t('Thêm phụ huynh') }} </a-button>

        <!-- Modal add user -->
        <a-modal v-model="visibleAdd" :footer="false" :title="$t('Thêm phụ huynh')" class="modal-admin" @cancel="handleCloseModalAdd">
          <template slot="closeIcon">
            <i class="fal fa-times icon"></i>
          </template>
          <a-tabs default-active-key="1">
            <!-- Tab thêm 1 user -->
            <a-tab-pane key="1" :tab="$t('Thêm 1 phụ huynh')">
              <a-form-model ref="ruleFormAdd" :model="formAdd" :rules="rules">
                <a-form-model-item ref="studenID" prop="studenID" label="studenID">
    ...
    columns() {
      const columns = [
        ...
        },
        {
          ...
          onFilter: (value, record) => record.nameChild.toString().toLowerCase().includes(value.toLowerCase()),
          sorter: (a, b) => a.nameChild.localeCompare(b.nameChild),
        },
        {
  ...
  methods: {
    ...
    async handleAddFileUser() {
      try {
        ...
        const result = await this.$axios.$post('/admin/addUsers', formData)
        console.log(result)
