...
export default {
  ...
  async fetch() {
    try {
      const result = await this.$axios.$get('/decks/' + this.$route.params.id)
      if (result.success) {
  ...
  methods: {
    async handleLike() {
      try {
        const result = await this.$axios.$post('/decks/' + this.$route.params.id)
        if (result.success) {
          this.fetchLike()
        }
      } catch (error) {
    ...
    handleSave() {
      this.$axios
        .$post('/api/auth/user/' + this.deck._id)
        .then((res) => {
          if (res.success) {
